...
import { UNKNOWN_PROJECT_REFERENCE } from './<%= language-id %>-package-manager.js';
...
export class QualifiedNameProvider implements NameProvider {
   ...
   getLocalName(node?: AstNode): string | undefined {
      return node
        ? isIdAstNode(node)
          ? node[properties.referenceProperty]
          : isNamed(node)
          ? node.name
          : undefined
        : undefined;
   ...
   getNameNode(node: AstNode): CstNode | undefined {
      return findNodeForProperty(node.$cstNode, '<%= ReferenceProperty %>');
   }
}
