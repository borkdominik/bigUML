...
const { shared, <%= LanguageName %> } = create<%= LanguageName %>Services({
  connection,
  ...NodeFileSystem,
});
...
shared.workspace.WorkspaceManager.onWorkspaceInitialized((workspaceFolders) => {
  ...
  // startGLSPServer({ shared, language: <%= LanguageName %> }, workspaceFolders[0]);
  // Start the JSON server with the shared services
  startModelServer({ shared, language: <%= LanguageName %> }, workspaceFolders[0]);
});
