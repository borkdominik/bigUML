...
import { CrossReference } from "generator-language-server";
...
export class <%= LanguageName %>JsonSerializer implements JsonSerializer {
  ...
  serialize(node: AstNode, options?: JsonSerializeOptions): string {
    ...
    const defaultReplacer = (key: string, value: unknown) =>
      this.replacer(key, value, options);
    const replacer = specificReplacer
      ? (key: string, value: unknown) =>
          specificReplacer(key, value, defaultReplacer)
      : defaultReplacer;
  ...
  protected replacer(
    key: string,
    value: unknown,
    { refText, sourceText, textRegions }: JsonSerializeOptions = {}
  ): unknown {
      ...
      if (refValue) {
        if ((refValue as any)[properties.referenceProperty]) {
          return {
            $refText,
            $ref: {
              __documentUri: value.$nodeDescription?.node
                ? undefined
                : value.$nodeDescription?.documentUri.path,
              <%= ReferenceProperty %>: (refValue as any)[properties.referenceProperty],
            },
  ...
  protected addAstNodeRegionWithAssignmentsTo(node: AstNodeWithTextRegion) {
    const createDocumentSegment: (
      cstNode: CstNode
    ) => AstNodeRegionWithAssignments = (cstNode) =>
      <DocumentSegment>{
    ...
    if (node.$cstNode) {
      const textRegion = (node.$textRegion = createDocumentSegment(
        node.$cstNode
      ));
      const assignments: Record<string, DocumentSegment[]> =
        (textRegion.assignments = {});

      Object.keys(node)
        .filter((key) => !key.startsWith("$"))
        .forEach((key) => {
          const propertyAssignments = findNodesForProperty(
            node.$cstNode,
            key
          ).map(createDocumentSegment);
      ...
      if (Array.isArray(item)) {
        for (let index = 0; index < item.length; index++) {
          ...
          if (isIntermediateReference(element)) {
            item[index] = this.reviveReference(
              node,
              propertyName,
              root,
              element
        ...
        node[propertyName] = this.reviveReference(
          node,
          propertyName,
          root,
          item
  ...
  protected getRefNode<T extends AstNode>(
    root: AstNode,
    ref: CrossReference<T>
  ): AstNode {
    if (ref[properties.referenceProperty] as string) {
      if (ref.__documentUri) {
        const doc = this.langiumDocs.getOrCreateDocument(
          URI.parse(ref.__documentUri)
        );
  ...
  private getAstNodeById<T extends AstNode = AstNode>(
    node: AstNode,
    id: string
  ): T | undefined {
