...
import {
  HighlightElementActionResponse,
  RequestHighlightElementAction,
} from "../common/highlight.action.js";
...
export class AdvancedSearchActionHandler implements Disposable {
  ...
  protected init(): void {
    this.toDispose.push(
      this.actionListener.handleVSCodeRequest<RequestAdvancedSearchAction>(
        RequestAdvancedSearchAction.KIND,
        (request) => {
          ...
          if (model) {
            ...
            const applicableMatchers = !type
              ? this.matchers
              : this.matchers.filter((m) =>
                  m.supportsPartial?.(type as string),
                );
      ...
      this.actionListener.handleVSCodeRequest<RequestHighlightElementAction>(
        RequestHighlightElementAction.KIND,
        (message) => {
