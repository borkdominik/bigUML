...
import {
  BIGReactWebview,
  type BIGWebviewProviderContext,
...
export class RevisionManagementProvider extends BIGReactWebview {
  ...
  protected override init(): void {
    ...
    this.toDispose.push(
      this.actionListener.handleVSCodeRequest(
        RequestExportSnapshotAction.KIND,
        async () => {
          console.log(
    ...
    this.toDispose.push(
      this.actionListener.handleVSCodeRequest(
        RequestSaveFileAction.KIND,
        async () => {
          console.log(
            "[RevisionManagementProvider] RequestSaveFileAction received",
          );
    ...
    this.toDispose.push(
      this.actionListener.handleVSCodeRequest(
        RequestImportSnapshotAction.KIND,
        async (message: any) => {
          console.log(
            "[RevisionManagementProvider] ImportSnapshot action received",
          );
    ...
    this.toDispose.push(
      this.actionListener.handleVSCodeRequest(
        RequestChangeSnapshotNameAction.KIND,
        async (message: any) => {
          console.log(
            "[RevisionManagementProvider] RequestChangeSnapshotNameAction action received",
          );
          await this.service.changeSnapshotName(
            message.action.snapshotId,
            message.action.name,
          );
          return { kind: "noop" } as any;
        },
    ...
    this.toDispose.push(
      this.actionListener.handleVSCodeRequest(
        RequestRestoreSnapshotAction.KIND,
        async (message) => {
          const action = message.action as RequestRestoreSnapshotAction;
          const snapshotId = action.snapshotId;

          console.log(
            `[RevisionManagementProvider] Restore request received for snapshot ID: ${snapshotId}`,
          );
    ...
    this.toDispose.push(
      this.actionListener.handleVSCodeRequest(
        RequestDeleteSnapshotAction.KIND,
        async (message) => {
          const action = message.action as RequestDeleteSnapshotAction;
          const snapshotId = action.snapshotId;

          console.log(
            `[RevisionManagementProvider] Delete request received for snapshot ID: ${snapshotId}`,
          );
