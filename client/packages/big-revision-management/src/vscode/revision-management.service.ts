...
import {
  ...
  RequestMinimapExportSvgAction,
} from "@borkdominik-biguml/big-minimap";
import {
  EXPERIMENTAL_TYPES,
  TYPES,
...
export class RevisionManagementService {
  ...
  protected readonly onDidChangeTimelineEmitter = new vscode.EventEmitter<
    Snapshot[]
  >();
  ...
  protected init(): void {
    ...
    this.toDispose.push(
      umlWatcher.onDidChange(async (uri) => {
        ...
        console.log(
          "[Snapshot] Triggering exportSvg via RequestMinimapExportSvgAction",
        );
    ...
    this.toDispose.push(
      this.connector.onClientActionMessage((message: any) => {
        if (
          MinimapExportSvgAction.is(message.action) &&
          this.svgRequestId !== null
        ) {
          console.log(
            "[RevisionManagementService] Received MinimapExportSvgAction message:",
            message,
          );
          const timelineEntry = this.timeline.find(
            (s) => s.id === this.svgRequestId,
          );
          if (timelineEntry) {
  ...
  protected handleConnection(): void {
    ...
    if (state) {
      console.log(
        "[RevisionManagementService] Initial model state loaded via getModelState",
      );
      this.currentModelState = state;

      const key = this.getTimelineKey();
      const stored =
        this.extensionContext.globalState.get<Snapshot[]>(key) ?? [];
      console.log(
        "[Timeline] Loaded from globalState for",
        key,
        ":",
        stored.length,
      );
      ...
      console.log(
        "[RevisionManagementService] No initial model state available",
      );
    }
    ...
    this.toDispose.push(
      ...
      this.modelState.onDidChangeModelState((event) => {
        console.log(
          "RevisionManagementService onDidChangeModelState",
          event.state,
        );
  ...
  async deleteSnapshot(snapshotId: string): Promise<void> {
    ...
    if (snapshotIndex === -1) {
      console.warn(
        `[RevisionManagementService] Snapshot with ID ${snapshotId} not found`,
      );
      return;
    }
    this.timeline = [
  ...
  async restoreSnapshot(snapshotId: string): Promise<void> {
    ...
    if (snapshotIndex !== -1) {
      ...
      console.log(
        `[RevisionManagementService] Restoring snapshot with ID ${snapshotId}:`,
        snapshot,
      );
      ...
      await this.extensionContext.globalState.update(key, this.timeline);
      console.log(
        `[RevisionManagementService] Timeline after restore saved for key: ${key} (entries: ${this.timeline.length})`,
      );
  ...
  createSnapshot(message: string): void {
    ...
    const affectedResources = this.currentModelState
      .getResources()
      .filter((resource) => resource.format === "xml");
    ...
    this.connector.sendActionToActiveClient(
      RequestMinimapExportSvgAction.create(),
    );
  }
