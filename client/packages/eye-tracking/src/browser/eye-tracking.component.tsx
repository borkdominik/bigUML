...
import { EyeTrackingActionResponse, RequestEyeTrackingAction } from '../common/index.js';
...
export function EyeTracking(): ReactElement {
    ...
    useEffect(() => {
        listenAction(action => {
            if (EyeTrackingActionResponse.is(action)) {
    ...
    const increase1 = useCallback(() => {
        dispatchAction(RequestEyeTrackingAction.create({ increase: 1 }));
    }, [dispatchAction]);

    const increase5 = useCallback(() => {
        dispatchAction(RequestEyeTrackingAction.create({ increase: 5 }));
    }, [dispatchAction]);

    return (
        <div>
            <span>Eye Tracking! {count}</span>
