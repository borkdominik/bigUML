...
import { parseEcoreDefinitionFile } from './ecore/ecore-parser.js';
...
if (args[0] === 'generate') {
    ...
    if (args.length > 1) {
        if (args.includes('--from=ecore')) {
            ...
            const ecorePath = path.resolve(extensionPath, 'definition', 'definition.ecore');
            parseEcoreDefinitionFile(ecorePath).then(() => {
                parseDefinitionFile(path.resolve(extensionPath, 'definition', 'def.ts')).then(tsDeclarations => {
                    console.log('DEBUG: Raw declarations from def.ts:', JSON.stringify(tsDeclarations, null, 2));
