...
class LangiumGenerator extends Generator {
    ...
    this.answers = await this.prompt([
      ...
      },
      {
        ...
        validate: (input: string): boolean | string =>
          /^[a-zA-Z].*$/.test(input)
            ? true
            : "The entry element name must start with a letter.",
      },
      {
        type: "input",
        name: "referenceProperty",
        prefix: description(
          "The property which is used to create the references"
        ),
        default: "__id",
  ...
  writing(): void {
    ...
    this.answers!.fileExtensions = `[${fileExtensions
      .map((ext) => `".${ext}"`)
      .join(", ")}]`;

    const fileExtensionGlob =
  ...
  _replaceTemplateWords(
    fileExtensionGlob: string,
    languageName: string,
    languageId: string,
    content: Buffer
  ): string {
