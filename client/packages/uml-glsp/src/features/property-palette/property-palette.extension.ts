...
import { createReferencePropertyNoCreate, ElementReferencePropertyItemNoCreate } from './choiceReference';
    ...
    protected refreshContent(items: ElementPropertyItem[]): void {
        ...
        if (items !== undefined && items.length > 0) {
            for (const propertyItem of items) {
                ...
                } else if (ElementReferencePropertyItemNoCreate.is(propertyItem)) {
                    created = createReferencePropertyNoCreate(
                        propertyItem,
                        {
                            onCreate: async (item, create) => {
                                this.disable();
