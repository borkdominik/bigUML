...
export function generateLangiumText(
  grammar: LangiumGrammar,
  _languageId: string = "grammar",
  languageName: string = "NewGrammar"
) {
...
function parserRuleToLangiumText(
  parserRules: Array<ParserRule>,
  langiumGrammar: LangiumGrammar
) {
  return parserRules
    .map((parserRuleElement) => getJsonRule(parserRuleElement, langiumGrammar))
    .join("\n");
}
function getJsonRule(
  rule: ParserRule | EntryRule,
  ...
  text.push(" '{' ");
  text.push(entry ? "" : `'""__"type"' ':' '"${rule.name}"' `);
  text.push(
    ` ${rule.definitions
      .map(
        (property, index) =>
...
function getProperty(property: Definition, rules: LangiumGrammar): string {
  ...
  if (property.multiplicity === Multiplicity.ONE_TO_ONE) {
    ...
    text.push(
      ")" + (property.multiplicity === Multiplicity.ZERO_TO_N ? "?" : "")
    );
    text.push("']'");
  }
  return text.join(" ");
...
function getReference(property: Definition): string[] {
  ...
  text.push(
    `'"'${property.name}${
      property.multiplicity === Multiplicity.ONE_TO_ONE ? "=" : "+="
    }[${property.type!.typeName}:LANGIUM_ID]'"'`
  );
  text.push(`'}'`);
  return text;
}
