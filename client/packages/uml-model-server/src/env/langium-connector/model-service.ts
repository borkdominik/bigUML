...
export class ModelService {
    ...
    async request<T extends AstNode>(uri: string, guard?: (item: unknown) => item is T, client?: string): Promise<AstNode | T | undefined> {
        if (client) {
    ...
    async update<T extends AstNode>(uri: string, model: T | string, client?: string): Promise<T> {
        ...
        if (client !== 'text' && version < this.documentManager.getClientDocumentVersion(uri, 'text')) {
            if (
                ...
                document.diagnostics.filter(value => value.data.code === 'linking-error').length === document.diagnostics.length
            ) {
                await this.documentManager.update(uri, this.documentManager.getClientDocumentVersion(uri, 'text'), text, client);
            } else {
