...
export interface IntermediatePatchRef<T> {
    type?: T;
...
export interface IntermediatePatchReference {
    $refText?: string;
    $ref?: IntermediatePatchRef<AstNode>;
    $error?: string;
}

export function isIntermediatePatchReference(obj: unknown): obj is IntermediatePatchReference {
        ...
        if (obj && typeof obj === 'object') {
            ...
            for (const key in obj) {
                if (Array.isArray(obj[key])) {
                    obj[key] = obj[key].filter((el: any) => !!el);
                }
                if (obj.hasOwnProperty(key)) {
                    visit(obj[key]);
                }
...
export function getReferenceUUID(json: any, ref: any) {
    if (ref.__id) {
        const nodes = findNodes(json, `__id`, ref.__id) ?? [];
        if (nodes[0]?.value) {
...
export function rebuildLangiumReferences(
    ...
    function linkNode(node: GenericAstNode, root: AstNode, container?: AstNode, containerProperty?: string, containerIndex?: number) {
        for (const [propertyName, item] of Object.entries(node)) {
            if (Array.isArray(item)) {
                for (let index = 0; index < item.length; index++) {
                    const element = item[index];
                    if (isIntermediatePatchReference(element)) {
            ...
            } else if (isIntermediatePatchReference(item)) {
    ...
    function reviveReference(
        ...
        property: string,
        root: AstNode,
        reference: IntermediatePatchReference
    ): Reference | undefined {
    ...
    function getRefNode<T extends AstNode>(root: AstNode, ref: IntermediatePatchRef<T>): AstNode {
        if (ref.__id) {
