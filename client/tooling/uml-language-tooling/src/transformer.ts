...
export function transformDeclaration(declarations: Array<Declaration>): Array<LangiumDeclaration> {
    ...
    const langiumDeclarations: Array<LangiumDeclaration> = declarations.map(declaration => {
        if (declaration?.extends!.length > 0) {
            declaration.extends!.forEach(extend => {
                declaration.properties = declaration.properties!.concat(declarations.find(d => d.name === extend)?.properties || []);
            });
        }
        return {
    ...
    declarations.forEach(declaration => {
        if (declaration.extends!.length > 0) {
            declaration.extends!.forEach(extend => {
                langiumDeclarations.find(langiumDeclaration => langiumDeclaration.name === extend)!.extendedBy!.push(declaration.name!);
            });
        }
    });
...
export function transformLangiumDeclarationToEntryRule(langiumDeclaration: LangiumDeclaration): EntryRule {
    return {
...
function typeRuleExists(typeRules: Array<TypeRule>, propertyTypes: any) {
    ...
    return Array.from(typeRules).some(typeRule => getSortedTypeNames(typeRule.definitions) === propertyTypesStr);
}

export function transformLangiumDeclarationsToTypeRules(langiumDeclarations: Array<LangiumDeclaration>): Array<TypeRule> {
        ...
        .forEach(declaration => {
            declaration.properties!.forEach(property => {
                if (property.types.length > 1) {
                    if (!typeRuleExists(typeRules as any, property.types) && !typeRuleExists(typeRules3, property.types)) {
                        typeRules3.push({
...
export function transformLangiumDeclarationsToLangiumGrammar(
    ...
    const entryRule = transformLangiumDeclarationToEntryRule(
        langiumDeclarations.find(langiumDeclaration => langiumDeclaration.decorators!.includes('root'))!
    );
    const typeRules = transformLangiumDeclarationsToTypeRules(langiumDeclarations);
    const parserRules = langiumDeclarations
        .filter(
            langiumDeclaration =>
                langiumDeclaration.type === 'class' && !langiumDeclaration.isAbstract && !langiumDeclaration.decorators!.includes('root')
        )
        .map(langiumDeclaration => {
            const properties: Array<Definition> = langiumDeclaration.properties!.map(property => ({
                name: property.name,
